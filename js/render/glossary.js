...
export function speak(text, btn) {
  if (!voiceReady) { toast('مرورگر تو از پخش صدا پشتیبانی نمی‌کند.', 'ph-speaker-slash'); return; }
  try {
  ...
  } catch (e) { toast('پخش صدا ممکن نشد.', 'ph-speaker-slash'); }
}
...
export function renderGlossary() {
  ...
  $('#crumbTitle').textContent = 'واژه‌نامهٔ Git';
  $('#root').innerHTML = `
    <div class="gl-head">
      <span class="gl-ic"><i class="ph-duotone ph-book-bookmark"></i></span>
      <div><h2 class="title" style="font-size:28px">واژه‌نامهٔ Git</h2>
      <p class="sub" style="margin-top:4px">${FA(GLOSSARY.length)} اصطلاح پرکاربرد، با معادل فارسی و لینک به سطحی که کامل توضیح داده شده.</p></div>
    </div>
    <div class="gl-wrap">
      <i class="ph ph-magnifying-glass"></i>
      <input class="gl-search" id="glSearch" type="search" placeholder="مثلاً: rebase، توکن، conflict…" value="${state.glQuery.replace(/"/g, '&quot;')}">
    </div>
  ...
  const paint = () => {
    ...
    count.textContent = q ? `${FA(items.length)} نتیجه` : `${FA(items.length)} اصطلاح`;
    list.innerHTML = items.length ? items.map(g => `
      <div class="gl-item">
        <div class="gl-top">
          <button class="say" data-say="${g.t.replace(/"/g, '&quot;')}" aria-label="شنیدن تلفظ ${g.t}"><i class="ph-fill ph-speaker-high"></i></button>
          <span class="gl-term">${g.t}</span>
          <span class="gl-fa">${g.fa}</span>
          <button class="gl-lv" data-lv="${g.lv}">سطح ${FA(g.lv)}</button>
        </div>
        ${g.p ? `<span class="gl-pron">تلفظ: <b>${g.p}</b></span>` : ''}
        <p>${g.d}</p>
      </div>`).join('')
      : `<p style="color:var(--muted);text-align:center;padding:26px 0">اصطلاحی با این عبارت پیدا نشد.</p>`;
    ...
    list.querySelectorAll('.gl-lv').forEach(b => b.addEventListener('click', () => {
      ...
      if (!isUnlocked(idx)) { toast('این سطح هنوز قفل است.'); return; }
      ctx.go(idx);
    }));
  };
