...
export const STRINGS = {
  ...
  'settings.btn':    { fa: 'تنظیمات دوره', en: 'Course settings' },
  'nav.system':      { fa: 'سیستم طراحی', en: 'Design system' },
  'system.badge':    { fa: 'پشت صحنهٔ رابط', en: 'Behind the interface' },
  'system.lead':     { fa: 'این صفحه، سیستم طراحی خودِ دوره است. همهٔ مقدارها همین لحظه از CSS زنده خوانده می‌شوند، پس هیچ‌وقت با کدِ واقعی اختلاف پیدا نمی‌کنند.', en: 'This page is the course’s own design system. Every value is read from the live CSS at render time, so it can never drift from the real code.' },
  'system.palette.h': { fa: 'رمپ تک‌رنگ', en: 'The monochrome ramp' },
  'system.palette.p': { fa: 'کل هویت رنگی دوره همین چهار خاکستری است. هر مرحلهٔ آموزشی یکی از آن‌ها را می‌گیرد و در نوار کناری، نشانه‌ها و نمودارها همان را تکرار می‌کند. رنگ، نقش «برند» ندارد؛ نقش «کجای مسیر هستی» را دارد.', en: 'The entire colour identity is these four greys. Each course phase takes one and repeats it across the rail, its markers and its charts. Colour is not branding here — it answers “where in the path am I?”' },
  ...
  'system.type.p':   { fa: 'متن اصلی روی حدود ۶۶ کاراکتر در خط تنظیم شده و ارتفاع خط ۱٫۹۵ است — فارسی به فضای عمودی بیشتری از لاتین نیاز دارد.', en: 'Body text is tuned to roughly 66 characters per line with a 1.95 line height — Persian needs more vertical space than Latin.' },
  ...
  'system.space.p':  { fa: 'یک مقیاس ۴ پیکسلی. هر فاصله در رابط از یکی از این توکن‌ها می‌آید، نه از عدد دستی.', en: 'A 4px scale. Every gap in the interface comes from one of these tokens, never a hand-typed number.' },
  ...
  'settings.resetNote': { fa: 'شروع دوباره، همهٔ پیشرفت این مرورگر را پاک می‌کند و برگشت‌پذیر نیست.', en: 'Restarting erases all progress stored in this browser. It cannot be undone.' },
  ...
  'intro.h2':           { fa: 'Git، برای طراحان محصول', en: 'Git, for product designers' },
  'intro.lead':         { fa: 'برای طراحان محصول و Design Technologistها: از «Commit یعنی چه؟» تا نسخه‌بندی Design System و بررسی کدی که هوش مصنوعی نوشته. هر مفهوم با زبان ساده، مثال واقعی طراحی و یک آزمون کوتاه که تا قبولی در آن، سطح بعدی باز نمی‌شود.', en: 'For product designers and design technologists: from “what is a commit?” to versioning a design system and reviewing AI-written code. Every concept in plain language, with a real design example and a short quiz — the next level stays locked until you pass it.' },
